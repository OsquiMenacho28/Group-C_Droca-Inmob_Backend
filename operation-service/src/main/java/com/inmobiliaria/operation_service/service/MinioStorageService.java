 ...
 *
 * <p>Responsibilities: - Upload a receipt file and return its object key. - Generate a pre-signed
 * GET URL so the frontend can download the file directly. - Delete a file from the bucket when a
 * receipt is removed.
 *
 * <p>Object key format: receipts/{operationId}/{uuid}.{extension} This keeps each operation's files
 * logically grouped inside the bucket.
 */
@Slf4j
@Service
public class MinioStorageService {
  ...
  /**
   * Generates a pre-signed GET URL for the given object key. The URL is valid for {@value
   * PRESIGNED_URL_EXPIRY_HOURS} hour(s).
   *
  ...
  /**
   * Deletes a file from MinIO by its object key. Errors are logged but not re-thrown — the database
   * record should still be deleted even if the MinIO object is already gone.
   *
  ...
  /**
   * Validates content type and file size. Throws {@link IllegalArgumentException} with a
   * user-friendly message on failure. This is the enforcement point for PA: "only valid receipt
   * formats accepted."
   */
  ...
  /** Builds the MinIO object key. Format: receipts/{operationId}/{uuid}.{extension} */
