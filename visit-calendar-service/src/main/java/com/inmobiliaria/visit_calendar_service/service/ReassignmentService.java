 ...
 *
 * <p>Flujo principal: 1. Agente original solicita reasignación → se crea SolicitudReasignacion en
 * estado PENDIENTE. 2. Agente destino acepta o rechaza → se actualiza el estado de la solicitud. -
 * Si ACEPTADA: se actualiza CalendarEvent.agentId al nuevo agente y se guarda el historial. - Si
 * RECHAZADA: la cita permanece sin cambios. 3. Se dispara notificación al agente solicitante con la
 * decisión.
 */
@Slf4j
@Service
public class ReassignmentService {
  ...
  /**
   * Aplica la reasignación en CalendarEvent: - Cambia agentId al agente destino - Actualiza
   * agentName con el nombre completo del nuevo agente
   */
  private void applyReassignment(ReassignmentRequest request) {
    ...
    // También actualizar la entidad Visit si existe para mantener consistencia y el
    // historial
    visitRepository
        .findById(request.getVisitId())
        .ifPresent(
            visit -> {
              ...
              log.debug(
                  "Historial de reasignación actualizado en la entidad Visit: {}", visit.getId());
            });
  }

  /**
   * Obtiene el nombre completo del agente consultando el microservicio de usuarios. Intenta primero
   * con user-service, luego con person-service como fallback.
   *
  ...
  private String getAgentFullName(String agentId) {
    ...
    try {
      ...
      log.warn(
          "No se pudo obtener el nombre del agente desde user-service para ID: {}. Intentando con person-service...",
          agentId,
          e);
    }
    ...
    try {
      ...
      log.warn(
          "No se pudo obtener el nombre del agente desde person-service para ID: {}", agentId, e);
    }
